...
COPY ./bin/start.sh ./bin/chown2root ./bin/mksmbdirs /usr/local/bin/
...
RUN apk add --no-cache samba-server sudo \
 ...
 && echo "samba ALL=(root) NOPASSWD: /usr/local/bin/chown2root /usr/local/bin/mksmbdirs" > /etc/sudoers.d/samba
