...
RUN apk add --no-cache samba-server sudo \
 ...
 && chmod 500 /usr/local/bin/sudo/* /usr/local/bin/runsmbd \
 && chmod go+rx /usr/local/bin/start.sh \
 && mkdir -p "$SECRET_DIR" \
 && touch "$SMBPASSWD_FILE" \
 && adduser -D -S -H -s /bin/false -u 100 samba \
 && chown samba "$CONFIG_DIR" "$SECRET_DIR" /usr/local/bin/runsmbd \
 && echo "samba ALL=(root) NOPASSWD: `ls -m /usr/local/bin/sudo`, /usr/sbin/nmbd, /usr/sbin/smbd" > /etc/sudoers.d/samba
