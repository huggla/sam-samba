...
RUN apk add --no-cache samba-server sudo \
 ...
 && tmp=$(find "$SUDO_DIR" -maxdepth 1 -type f -print0 | xargs -0 printf %q | sed "s/''/','/g") \
 && echo "$USER ALL=(root) NOPASSWD: `echo $tmp`, /usr/sbin/nmbd, /usr/sbin/smbd" > /etc/sudoers.d/samba
